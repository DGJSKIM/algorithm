...
public class AnagramGroup_2 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        HashMap<char[], Long > countMap = new HashMap<>();
        //일단 EOF 처리 ctrl D
        while ((input = br.readLine())!=null) {
            ...
            for (char[] charList : charArr) {
                if (Arrays.equals(inputChar, charList)) { // 같은게 있다면
                    ...
                    charSame = true;
                    // 카운트 ++
                    countMap.put(charList, countMap.get(charList)+1);
        ...
        List<Map.Entry<char[], ArrayList<String>>> entryList = charString.entrySet()
                ...
                .sorted((entry1, entry2)->{
                    int sizeCompare = Integer.compare(countMap.get(entry2.getKey()).intValue(),countMap.get(entry1.getKey()).intValue());
                    if(sizeCompare!=0){
                        return sizeCompare;
                    }
